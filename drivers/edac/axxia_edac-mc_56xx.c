		...
		for (i = 0; i < NR_EVENTS; ++i) {
			...
			if (counter)
				...
				case EV_MULT_OUT_OF_RANGE:
				{
					...
					u64 upper;
					/*
					 * Unfortunately this driver instead of using EDAC memory module APIs
					 * uses EDAC device module APIs devised for non-memory types.
					 * With a plan (hopefully getting to realized) of introducing
					 * an updated mechanism for handling EDAC (RAS, firmware-first handle,
					 * SDEI interface) no point to rewrite all, just stick extra details
					 * case by case where needed.
					 */
					edac_device_handle_multi_ue(edac_dev,
						0, i, counter,
						edac_dev->ctl_name);
					ncr_read(dev_info->sm_region, SM_56XX_DENALI_CTL_132, 4, &lower);
					ncr_read(dev_info->sm_region, SM_56XX_DENALI_CTL_133, 4, &tmp);
					upper = tmp & 0x3f;
					ncr_read(dev_info->sm_region, SM_56XX_DENALI_CTL_134, 4, &source);
					edac_device_printk(edac_dev, KERN_EMERG,
							"%s: out_of_range_addr 0x%llx out_of_range_length 0x%x",
							edac_dev->ctl_name, (unsigned long long) ((upper << 32) | lower),
							(tmp >> 8) & 0x1fff);
					printk(KERN_CONT " out_of_range_type 0x%x out_of_range_source_id 0x%x\n",
