...
#define ERR_SYND_FIRST_ERR_VLD 	(1ULL << 62)
...
irqreturn_t arm_ccn_error_handler(void *ptr)
{
	...
	for (instance = 0;
		...
		if (err_syndrome_reg0 & ERR_SYND_FIRST_ERR_VLD) {
			if (((err_syndrome_reg0 & ERR_SYND_ERR_CLASS) >> 60) == 0x3) {
			...
			count = (err_syndrome_reg0 & ERR_SYND_ERR_COUNT_SET) >> 8;
			if (count)
				edac_device_handle_multi_ce(dev_info->edac_dev, 0,
...
static int intel_edac_l3_probe(struct platform_device *pdev)
{
	...
	dev_info->irq_used = *(unsigned*) (&pdev->dev)->platform_data;
	...
	if (dev_info->irq_used) {
		struct error_reporting *er;
		dev_info->er = devm_kzalloc(&pdev->dev, sizeof(*dev_info->er), GFP_KERNEL);
		if (!dev_info->er)
			return -ENOMEM;
...
#define TEST_ERROR_FLOW 0
		if (TEST_ERROR_FLOW) {
			...
			dev_dbg(dev_info->dev, "injecting HNF error %08x @ %px\n",
					val,
					dev_info->dickens_L3 + CCN_HNF_NODE_BASE_ADDR(0) + 0x038);
			writel(val, dev_info->dickens_L3 + CCN_HNF_NODE_BASE_ADDR(0) + 0x038);
		}
	}
