...
unsigned long (*errint_read) (const volatile void __iomem *reg);
void (*errint_write) (u32 val, volatile void __iomem *addr);

static unsigned long secure_errint_read(const volatile void __iomem *reg)
{
...
static unsigned long nonsecure_errint_read(const volatile void __iomem *reg)
{
	return readl(reg);
}

static void secure_errint_write(u32 val, volatile void __iomem *addr)
{
	struct arm_smccc_res res = {0};
...
static void nonsecure_errint_write(u32 val, volatile void __iomem *addr)
{
	writel(val, addr);
}
...
static irqreturn_t arm_ccn_irq_handler(int irq, void *dev_id)
{
	 ...
	 * has to clear an MN error signal by reading Error Signal Valid regs and
	 * deassert the interrupt (INTREQ).
	 */
	for (i = 1; i < ARRAY_SIZE(err_sig_val); i++) {
		err_sig_val[i] = readl(ccn->base + CCN_MN_ERR_SIG_VAL_63_0 + i * 4);
...
#define TEST_ERROR_FLOW 0
		if (TEST_ERROR_FLOW) {
			...
			dev_dbg(ccn->dev, "injecting HNF error %08x @ %px\n",
					val,
